import tensorflow as tf
from tensorflow.examples.tutorials.mnist import input_data

mnist = input_data.read_data_sets('/temp/data', one_hot = True)

n_nodes_hl1 = 500
n_nodes_hl2 = 500
n_nodes_hl3 = 500

n_classes = 10
batch_size = 128
epochs_no = 10

x = tf.placeholder('float',[None, 784])
y = tf.placeholder('float')

def conv2d(x,W):
    #                       slide 1 overlap
    return tf.nn.conv2d(x, W, strides=[1,1,1,1], padding="SAME")

def maxpooling(x):
    #                       2*2 sizewidow       slide 2
    return tf.nn.max_pool(x, ksize=[1,2,2,1], strides= [1,2,2,1], padding="SAME")


def convolutional_network_model(x):
    #                                           5x5 1input 32filters
    weights = {'W_con1': tf.Variable(tf.random_normal([5,5,1,32])),
               'W_con2': tf.Variable(tf.random_normal([5,5,32,64])),
               'W_fc': tf.Variable(tf.random_normal([7*7*64,1024])),
               'W_out': tf.Variable(tf.random_normal([1024,n_classes])),}
    #                                           1024 fc node
    biases = {'b_con1': tf.Variable(tf.random_normal([32])),
              'b_con2': tf.Variable(tf.random_normal([64])),
              'b_fc': tf.Variable(tf.random_normal([1024])),
              'b_out': tf.Variable(tf.random_normal([n_classes])),}
    
    x = tf.reshape(x,shape =[-1,28,28,1])
    conv1 = tf.nn.relu(conv2d(x,weights['W_con1'])+biases['b_con1'])
    conv1 = maxpooling(conv1)
    
    conv2 = tf.nn.relu(conv2d(conv1,weights['W_con2'])+biases['b_con2'])
    conv2 = maxpooling(conv2) 
    
    fc = tf.reshape(conv2,[-1, 7*7*64])
    fc = tf.nn.relu(tf.matmul(fc, weights['W_fc'])+biases['b_fc'])
    
    fc = tf.nn.dropout(fc,0.8)
    
    output = tf.matmul(fc,weights['W_out'])+biases['b_out']

    return output

def train_neural_network(x):
    prediction = convolutional_network_model(x)
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=prediction, labels=y))   # v1.0 changes
      # optimizer value = 0.001, Adam similar to SGD
    optimizer = tf.train.AdamOptimizer().minimize(cost)
        
    init = tf.global_variables_initializer()

    with tf.Session() as sess:
        sess.run(init)   
        for epoch in range(epochs_no):
            epoch_loss = 0
            for _ in range(int(mnist.train.num_examples/batch_size)):
                epoch_x, epoch_y = mnist.train.next_batch(batch_size)
                _, c = sess.run([optimizer, cost], feed_dict = {x: epoch_x, y: epoch_y})
                # code that optimizes the weights & biases
                epoch_loss += c
            print('Epoch', epoch, 'of', epochs_no, 'error:', epoch_loss)
  
        # testing
        correct = tf.equal(tf.argmax(prediction, 1), tf.argmax(y, 1))
        accuracy = tf.reduce_mean(tf.cast(correct, 'float'))
        print('Accuracy:', accuracy.eval({x: mnist.test.images, y: mnist.test.labels}))
        
train_neural_network(x)
                
        
